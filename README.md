...
Every climbing spot on Earth on one interactive map. Next.js 16 + Leaflet,
...
`points.geojson` is built from OpenStreetMap (`sport=climbing`) via the
Overpass API and tagged with country borders from Natural Earth:

```bash
npm run data        # SITE inferred from package name
```
...
generic and shared with the sibling sites (sailing / fishing world maps).
